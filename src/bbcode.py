...
_BBCODE_WRAPPED_BLOCK_REGEX = re.compile(
    r"(?=(?P<block>\[(?P<tag>[a-z*][a-z0-9*]*)(?:[ =][^\]]*)?\]"
    r"(?P<body>(?:(?!\[(?P=tag)(?:[ =][^\]]*)?\])[\s\S])*?)\[/(?P=tag)\]))",
    flags=re.IGNORECASE,
)
_SECTION_LABEL_REGEX = re.compile(
    r"(?:screenshots?|screens|comparisons?|encode|source|before|after)(?:\s*[:\-\u2013\u2014]+)?\s*\Z",
    flags=re.IGNORECASE,
)
_COMPARISON_TAG_REGEX = re.compile(r"\[comparison(?:[ =][^\]]*)?\]", flags=re.IGNORECASE)
...
_DISTRIBUTION_INSTRUCTION_REGEX = re.compile(
    r"\b(?:do[\s-]+not[\s-]+(?:(?:re[\s-]*)?upload|re[\s-]*tag)|"
    r"do[\s-]+not[\s-]+(?:share|distribute|trade)|"
    r"not[\s-]+for[\s-]+(?:public|trade)|"
    r"not[\s-]+to[\s-]+be[\s-]+(?:uploaded|shared)|"
    r"no[\s-]+re[\s-]*tagging)\b",
    flags=re.IGNORECASE,
)
...
def _is_imported_signature(candidate: str, *, alignment_block: bool) -> bool:
    ...
    if "internal" in text and ("release" in text or "exclusiv" in text):
        return True
    # Short distribution instructions identify release-group branding without an attribution verb or tool name.
    if _DISTRIBUTION_INSTRUCTION_REGEX.search(text):
        return True
    leading = text
    for _ in range(4):
...
def strip_orphan_section_headings(desc: str) -> str:
    """Remove section labels that would falsely advertise images we stripped out.

    Only the OUTERMOST label-only block is considered: a container holding other content does not
    have label-only visible text, so nesting resolves itself without a second pass. Deleting the
    whole block leaves nothing for collapse_empty_wrappers to tidy in the common case, and an
    outer wrapper it can clear in the rest.

    A heading survives whenever anything substantive follows it, up to the next heading - an
    image, a comparison, or any prose. That is the definition of orphaned, not a safety net, so
    the image check stays even though the sole caller strips images just before.
    """
    headings: list[tuple[int, int]] = []
    for match in _BBCODE_WRAPPED_BLOCK_REGEX.finditer(desc):
        if not _SECTION_LABEL_REGEX.fullmatch(_plain_text(match.group("block"))):
            continue
        start, end = match.start("block"), match.end("block")
        if not any(start >= outer_start and end <= outer_end for outer_start, outer_end in headings):
            headings.append((start, end))
    headings.sort()

    removals: list[tuple[int, int]] = []
    for index, (start, end) in enumerate(headings):
        region = desc[end : headings[index + 1][0] if index + 1 < len(headings) else len(desc)]
        if _IMAGE_TAG_REGEX.search(region) or _COMPARISON_TAG_REGEX.search(region):
            continue
        if any(character.isalnum() for character in _plain_text(region)):
            continue
        removals.append((start, end))

    for start, end in reversed(removals):
        desc = f"{desc[:start]}{desc[end:]}"
    return desc
...
class BBCODE:
    ...
    def clean_unit3d_description(self, desc: str, site: str) -> tuple[str, list[dict[str, Any]]]:
        ...
        # Remove leftover [img] or [URL] tags in the description
        desc = _IMAGE_TAG_REGEX.sub("", desc)
        desc = strip_orphan_section_headings(desc)
        desc = collapse_empty_wrappers(desc)
